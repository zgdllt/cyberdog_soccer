...
async def main(args=None):
    ...
    # 等待直到收到有效的位置消息
    opponent_location_subscriber1 = LocationSubscriber('other_location_subscriber','Seymour')
    opponent_location_subscriber2 = LocationSubscriber('other_location_subscriber2','Seymour')
    self_location_subscriber = LocationSubscriber('self_location_subscriber','betago2')
    teammate_location_subscriber = LocationSubscriber('teammate_location_subscriber','Seymour')
    ...
    while abs(ballpos.y)<abs(goalpos.y):
        ...
        while math.hypot(ballpos.x-mypos.x,ballpos.y-mypos.y) > 0.4:
            ...
            for pos in positions:
                ...
                if cos[-1] > 0.8 and math.hypot(ot_vector[-1].x,ot_vector[-1].y) < math.hypot(vector.x,vector.y):
                    not_safe = True
                    break
            # print("cos: ", cos)
            if not_safe:
                ...
                # move_node.change_speed(0.0,0.0,angle-myangle)
            else:
